...
import { showPlayerByGreaterRuns } from '../Redux'
...
function FindByRuns () {
  ...
  const runs = param.runs

  useEffect(() => {
    PlayerService.showPlayerByGreaterRuns(runs)
      .then((response) => {
        dispatch(showPlayerByGreaterRuns(response.data))
      })
      .catch((error) => {
        console.log(error)
      })
